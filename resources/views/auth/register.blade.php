@extends('layouts.app', ['class' => 'login-page', 'page' => _('Register Page'), 'contentClass' => 'login-page'])
        ...
        <form class="form" method="post" action="{{ route('register') }}">
            ...
            <div class="card card-login card-white">
                <div class="card-header">
                    ...
                    <h1 class="card-title">{{ _('Register') }}</h1>
                </div>
                <div class="card-body">
                    ...
                    <div class="input-group{{ $errors->has('password_confirmation ') ? ' has-danger' : '' }}">
                        ...
                        <input type="password" placeholder="{{ _('Confirm Password') }}" name="password_confirmation" class="form-control{{ $errors->has('password_confirmation ') ? ' is-invalid' : '' }}">
                        @include('alerts.feedback', ['field' => 'password_confirmation '])
                    </div>
                </div>
                <div class="card-footer">
                    <button type="submit" href="" class="btn btn-primary btn-lg btn-block mb-3">{{ _('Register') }}</button>
                    <div class="pull-left">
                        <h6>
                            <a href="{{ route('login') }}" class="link footer-link">{{ _('Login') }}</a>
                        </h6>
                    </div>
